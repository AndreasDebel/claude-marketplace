...
Re-explain the same true thing in words that reach a smart non-specialist. Opposite trade-off from `/simple`: extra words are fine here as long as each one earns its place. Length isn't the goal — landing is.

## Target

Default: your own last substantive response. If the user names a part ("/easy the AGP bit"), zoom into just that. If there's nothing to re-explain — a fresh question, or a fresh conversation — answer directly, written in this style from the start.
